...
interface ServicesPanelProps {
  ...
  readonly onSave: (values: Pick<Service, "nome" | "duracaoMin" | "preco" | "profissionalId" | "ativo"> & { id?: string }) => Promise<void>;
  readonly onDelete: (id: string) => Promise<void>;
  readonly canManage?: boolean;
}

export function ServicesPanel({ services, professionals, onSave, onDelete, canManage = true }: ServicesPanelProps) {
    ...
    <SectionCard title="Serviços" description="Procedimentos e consultas oferecidos pela clínica.">
      <div className="mb-4 grid gap-3 md:grid-cols-[1fr_240px]">
        ...
        <Field label="Profissional">
          <select className={inputClass()} value={filters.professionalId} onChange={(event) => setFilters({ ...filters, professionalId: event.target.value })}>
      ...
      {canManage ? (
        <form className="mb-5 grid gap-3 md:grid-cols-[1fr_150px_150px_1fr_auto]" onSubmit={(event) => {
          event.preventDefault();
          void onSave({ ...form, ativo: true });
          setForm({ id: "", nome: "", duracaoMin: 30, preco: 150, profissionalId: professionals[0]?.id ?? "" });
        }}>
          ...
          <button className="mt-5 inline-flex h-10 items-center justify-center gap-2 rounded-lg bg-primary px-4 text-sm font-medium text-white transition hover:bg-primary-dark" type="submit">
            <Plus className="h-4 w-4" />
            {form.id ? "Atualizar" : "Adicionar"}
          </button>
        </form>
      ) : (
        <div className="mb-5 rounded-lg border border-surface-variant bg-surface-container-lowest px-4 py-3 text-sm text-secondary">
          Você pode consultar os serviços cadastrados. Cadastro, edição e preços são restritos ao admin.
        </div>
      )}

      {filteredServices.length === 0 ? (
        <EmptyState title="Nenhum serviço cadastrado" message="Crie um serviço para liberar agendamentos na clínica." />
      ) : (
        <RefinedTable headers={canManage ? ["Serviço", "Profissional", "Duração", "Preço", "Ações"] : ["Serviço", "Profissional", "Duração", "Preço"]}>
          {filteredServices.map((service) => (
            <tr className="border-b border-surface-variant transition hover:bg-teal-50/60" key={service.id}>
              <td className="px-4 py-3 font-medium">{service.nome}</td>
              <td className="px-4 py-3 text-secondary">{service.profissionalNome ?? "-"}</td>
              <td className="px-4 py-3 text-secondary">{service.duracaoMin} min</td>
              <td className="px-4 py-3 text-right font-semibold text-on-surface">{brl.format(service.preco)}</td>
              {canManage ? (
