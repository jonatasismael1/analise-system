          ...
          {filteredProfessionals.map((professional) => (
            <article className="flex items-center justify-between rounded-lg border border-surface-variant bg-white p-4" key={professional.id}>
              ...
              <div className="flex gap-2">
                ...
                <button className="rounded p-2 text-secondary hover:bg-red-50 hover:text-error" onClick={() => { if (confirmDangerAction(`Excluir o profissional ${professional.nome}?`)) void onDelete(professional.id); }} type="button"><Trash2 className="h-4 w-4" /></button>
              </div>
            </article>
          ))}
        </div>
